...
export async function fetchVendorModels({ vendor, filters = {} }) {
  ...
  const vendorWithHyphen = vendor.toLowerCase().replace(/\s+/g, '-');

  // =================== משיכת שלד הדגמים מתוך ה-Metaobjects ===================
  ...
  if (metaData?.metaobjects?.edges) {
    metaData.metaobjects.edges.forEach(({ node }) => {
      ...
      // 🌟 התיקון המדויק: מסננים את ה-Metaobject לפי ה-displayName שמופיע בשופיפיי
      if (
        displayNameLower.startsWith(normalizedVendorParam) || 
        displayNameLower.startsWith(vendorWithHyphen) ||
        displayNameLower.startsWith(vendor.toLowerCase()) ||
        displayNameLower.startsWith(cleanVendor.toLowerCase())
      ) {
        let tagCode = null;
        if (node.fields) {
          node.fields.forEach((f) => {
            if (f.key === 'tag_code') tagCode = f.value;
          });
        }

        // חילוץ שם הדגם הנקי (למשל "HUSQVARNA TE 150" הופך ל-"TE 150")
        let modelName = node.displayName || '';
        const removeVendorRegex = new RegExp(`^${cleanVendor}|^${vendorNoSpaces}|^${vendorWithHyphen}`, 'i');
