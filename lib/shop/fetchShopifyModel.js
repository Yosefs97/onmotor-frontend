...
export async function fetchShopifyModel({ vendor, model, filters = {} }) {
  ...
  // ציד מדויק: נבקש משופיפיי כל מוצר שמכיל גם את שם היצרן וגם את שם הדגם בכל מקום שהוא (כולל בתגיות)
  const searchQuery = `${cleanVendor} ${cleanModel}`;
  ...
  try {
    ...
    // מנגנון ה-JavaScript המדויק שמוודא התאמה ספציפית ב-100%
