...
export async function fetchForumCategories() {
  ...
  console.log("📁 קטגוריות שהתקבלו:", json.data); // 🔍 חשוב לבדיקה
  ...
  return (
    json.data?.map((item) => {
      ...
      return {
        ...
        name: attrs.name || "ללא שם",
        slug: attrs.slug || item.slug || "", // 🧩 כאן התיקון
...
/* 🟨 פתיחת שרשור חדש לפי slug קטגוריה (תיקון מלא עם לוגים) */
export async function addThread({ title, content, author, categorySlug }) {
  try {
    const decodedSlug = decodeURIComponent(categorySlug?.trim());
    console.log("📌 addThread התחיל עם:", { title, decodedSlug });

    // 🟢 שלב 1: בדיקת קטגוריה ב-Strapi
    const catRes = await fetch(
      `${API_URL}/api/forum-categories?filters[slug][$eq]=${decodedSlug}&populate=*`,
      { cache: "no-store" }
    );

    if (!catRes.ok) {
      console.error("❌ שגיאה בקריאת קטגוריה:", catRes.status, catRes.statusText);
      throw new Error("שגיאה בגישה לקטגוריה");
    }

    const catJson = await catRes.json();
    console.log("📁 תוצאת חיפוש קטגוריה:", catJson);

    const categoryId = catJson?.data?.[0]?.id;
    if (!categoryId) {
      console.error("❌ לא נמצאה קטגוריה תואמת ל-slug:", decodedSlug);
      throw new Error("לא נמצאה קטגוריה מתאימה ל-slug");
    }

    // 🟢 שלב 2: יצירת slug לדיון
    ...
    const responseText = await res.text();
    console.log("📩 תגובת Strapi (Thread):", responseText);
    ...
    const json = JSON.parse(responseText);
    console.log("✅ דיון נוצר בהצלחה:", json);
    return json.data;
  } catch (err) {
