  ...
  return (
    json.data?.map((item) => {
      ...
      return {
        ...
        // Strapi v5 מחזיר את השדות גם בטופ־לבל וגם בתוך attributes
...
/* 🟨 פתיחת שרשור חדש לפי slug קטגוריה (תיקון מלא) */
export async function addThread({ title, content, author, categorySlug }) {
  try {
    const decodedSlug = decodeURIComponent(categorySlug || "").trim();
    console.log("📌 addThread התחיל עם:", { title, categorySlug, decodedSlug });

    // שלב 1 – מציאת קטגוריה
    const catRes = await fetch(
      `${API_URL}/api/forum-categories?filters[slug][$eq]=${decodedSlug}`,
      { cache: "no-store" }
    );
    ...
    const body = {
      data: {
        ...
        author,
        // Strapi v5 – קשר manyToOne בעזרת connect
        category: {
          connect: [categoryId],
        },
        pinned: false,
