...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    // 🟢 קודם נשלוף את ה-id של הדיון לפי ה-slug
    ...
    if (!threadId) {
      console.warn("⚠️ לא נמצא דיון עבור slug:", decodedSlug);
      return [];
    }

    // 🔥 עכשיו נשתמש ב-id כדי לשלוף תגובות
    const query = new URLSearchParams({
      "filters[forum-thread][id][$eq]": threadId,
      populate: "forum-thread,reply_to",
      sort: "createdAt:asc",
    });

    const res = await fetch(`${API_URL}/api/forum-comments?${query}`, {
      cache: "no-store",
    });

    if (!res.ok) throw new Error("שגיאה בטעינת תגובות לפי id");
    const json = await res.json();

    console.log("💬 תגובות מהשרת לפי id:", JSON.stringify(json.data, null, 2));

    return (
      json.data?.map((item) => {
        const ca = item.attributes || {};
        const rawAuthor =
          ca.author ??
          ca.user?.username ??
          ca.user?.name ??
...
export async function addCommentByThreadSlug({
  ...
  // 🟢 שולפים את מזהה הדיון לפי ה-slug
  const threadRes = await fetch(
    `${API_URL}/api/forum-threads?filters[slug][$eq]=${decodedSlug}`
  );
  const threadJson = await threadRes.json();
  ...
  // ✅ גוף נתונים בפורמט הנכון לפי Strapi 5 (manyToOne)
  const body = {
    data: {
      text,
      author,
      "forum-thread": threadId, // ✅ מזהה פשוט
  ...
  // 🕓 השהיה קצרה לוודא שהתגובה נרשמה
  await new Promise((r) => setTimeout(r, 500));

  // 🔢 עדכון מונה תגובות
  try {
    const threadRes2 = await fetch(
      `${API_URL}/api/forum-threads/${threadId}?populate=comments`
    );
    const threadJson2 = await threadRes2.json();
    const currentCount =
      threadJson2?.data?.attributes?.comments?.data?.length || 0;
  ...
  console.log("✅ תגובה נוצרה בהצלחה");
  return true;
}
