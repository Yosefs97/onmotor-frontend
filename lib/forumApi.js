...
/* 🟨 קבלת שרשור בודד לפי slug (תיקון מלא) */
export async function fetchThreadBySlug(threadSlug) {
  try {
    ...
    // 🧠 אם לא נמצא לפי slug — ננסה לפי id (למקרה של UID בעייתי)
    if (!item && !isNaN(threadSlug)) {
      console.warn("⚠️ לא נמצא לפי slug, מנסה לפי ID...");
      res = await fetch(`${API_URL}/api/forum-threads/${threadSlug}?populate=comments,comments.reply_to,category`);
      if (res.ok) {
