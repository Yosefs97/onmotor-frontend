...
export async function fetchThreadBySlug(threadSlug) {
  try {
    ...
    const proxyUrl = `/api/forum-thread?slug=${encodeURIComponent(decodedSlug)}`;
    ...
    return {
      ...
      comments:
        attrs.comments?.data?.map((c) => ({
          ...
          author: c.attributes?.author || "אנונימי",
          date:
            c.attributes?.date ||
...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    const query = new URLSearchParams({
      ...
      populate: "*", // ✅ שולף את כל השדות כולל reply_to
      sort: "createdAt:asc",
    });
...
export async function addCommentByThreadSlug({
  ...
  const threadRes = await fetch(
    `${API_URL}/api/forum-threads?${query.toString()}`
  );
  ...
  if (!res.ok) throw new Error(`שגיאה בשליחת תגובה (${res.status})`);
  ...
  // ✅ נוודא שהתגובה נשמרה לפני עדכון הספירה
  ...
  try {
    ...
    const threadJson = await threadRes.json();
    const currentCount =
      threadJson?.data?.attributes?.comments?.data?.length || 0;
...
export async function addThread({ title, content, author, categorySlug }) {
  try {
    ...
    const responseText = await res.text();
    if (!res.ok)
      throw new Error(`שגיאה ביצירת דיון חדש (${res.status}) - ${responseText}`);
