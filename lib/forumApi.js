...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    const query = new URLSearchParams({
      ...
      populate: "*", // 🟢 חשוב: שולף את כל השדות כולל reply_to
      sort: "createdAt:asc",
    });
    ...
    console.log("💬 תגובות שהתקבלו:", JSON.stringify(json.data, null, 2));
...
export async function addCommentByThreadSlug({
  ...
  // 🟢 שליפת מזהה דיון לפי slug
  const threadRes = await fetch(`${API_URL}/api/forum-threads?${query}`);
  ...
  // ✅ גוף נתונים מעודכן לפי מבנה Strapi 5
  const body = {
    data: {
      text,
      author,
      forum_thread: { connect: [threadId] }, // ✅ חיבור תקין לדיון
      reply_to: reply_to ? { connect: [reply_to] } : null, // ✅ חיבור לתגובה קודמת אם יש
  ...
  // 🕓 השהיה קצרה לוודא שהתגובה נשמרה
  await new Promise((r) => setTimeout(r, 500));

  // 🔢 עדכון ספירת תגובות לאחר יצירת תגובה
  try {
