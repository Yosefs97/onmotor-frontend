  ...
  return (
    json.data?.map((item) => {
      ...
      return {
        ...
        locked: attrs.locked,
        views: attrs.views || 0,
        lastActivity: attrs.lastActivity || attrs.updatedAt || attrs.createdAt || null,
...
try {
  const threadRes = await fetch(`${API_URL}/api/forum-threads/${threadId}?populate=comments`);
  const threadJson = await threadRes.json();
  const currentCount = threadJson?.data?.attributes?.comments?.data?.length || 0;
