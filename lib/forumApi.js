...
export async function fetchThreadBySlug(threadSlug) {
  try {
    ...
    // 🔹 ניסיון שני: לפי documentId (עובד גם כשslug הוא UID)
    if (!item) {
      console.warn("⚠️ לא נמצא לפי slug, מנסה לפי documentId...");
      const queryDoc = new URLSearchParams({
        "filters[documentId][$eq]": decodedSlug,
        populate: "comments,comments.reply_to,category",
      });
      const res2 = await fetch(`${API_URL}/api/forum-threads?${queryDoc.toString()}`, {
        cache: "no-store",
      });
      if (res2.ok) {
