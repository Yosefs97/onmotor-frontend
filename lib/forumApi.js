...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    // נחפש לפי ה־slug של ה־thread ישירות באוסף התגובות
    const query = new URLSearchParams({
      "filters[forum_thread][slug][$eq]": decodedSlug,
      "populate[forum_thread][fields][0]": "slug",
      "populate[forum_thread][fields][1]": "title",
      "populate[reply_to][fields][0]": "id",
      "sort[0]": "createdAt:asc",
      "pagination[pageSize]": "100",
    });

    const url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/forum-comments?${query.toString()}`;
    ...
    // 🧩 נבנה את מבנה התגובות לפי Strapi 5
    const comments = (json.data || []).map((item) => {
      const a = item.attributes || {};
      return {
        id: item.id,
        author: a.author || "אנונימי",
        text: a.text || "— אין תוכן —",
        date: a.date || a.createdAt || "",
        createdAt: a.createdAt,
        reply_to: a.reply_to?.data?.id || null,
      };
    });
