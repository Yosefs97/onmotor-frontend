...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/forum-comments?${query.toString()}`, { cache: "no-store" });
    if (!res.ok) throw new Error(`שגיאה בטעינת תגובות (${res.status})`);

    const json = await res.json();

    // 🧩 נוודא שהפונקציה קוראת את הנתונים ממבנה Strapi 5
    const comments = (json.data || []).map((item) => {
      const attr = item.attributes || {};
      return {
        id: item.id,
        text: attr.text || '',
        author: attr.author || 'אנונימי',
        date: attr.date || attr.createdAt || '',
        reply_to: attr.reply_to?.data?.id || attr.reply_to?.id || null,
