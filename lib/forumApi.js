...
/* 🟨 קבלת שרשור בודד לפי slug בלבד */
/* 🟨 קבלת שרשור בודד לפי slug (מותאם ל־UID בסגנון thread-xxxx) */
export async function fetchThreadBySlug(threadSlug) {
  try {
    const decodedSlug = decodeURIComponent(threadSlug || "").trim();
    console.log("📎 טעינת דיון לפי slug:", decodedSlug);

    // 🧠 תומך ב-UID שנוצר אוטומטית (thread-xxxx)
    ...
    if (!res.ok) {
      console.error("❌ fetchThreadBySlug שגיאה בשרת:", res.status, res.statusText);
