...
export async function fetchThreadBySlug(threadSlug) {
  try {
    ...
    // ✅ תמיכה גם ב-Strapi v5 וגם ב-v4
    const attrs = item.attributes ? item.attributes : item;
    console.log("✅ thread מוכן להחזרה:", item);
    ...
    return {
      ...
      comments:
        attrs.comments?.data?.map((c) => ({
          id: c.id,
          text: c.attributes?.text || c.text,
          author: c.attributes?.author || c.author,
          date: c.attributes?.date || c.createdAt,
          reply_to:
            c.attributes?.reply_to?.data?.id ||
            c.reply_to?.id ||
            null,
        })) || [],
    };
