...
export async function fetchThreadsByCategorySlug(slug) {
  ...
  const res = await fetch(
    `${API_URL}/api/forum-threads?filters[category][slug][$eq]=${decodedSlug}&sort=pinned:desc,lastActivity:desc&populate[comments][fields][0]=id`,
    { cache: "no-store" }
  );
  ...
  console.log("📦 forum threads:", json.data);
  ...
  return (
    json.data?.map((item) => {
      ...
      return {
        ...
        // ✅ עכשיו זה יעבוד — Strapi מחזיר comments.data
        commentsCount: attrs.commentsCount ?? 0,
...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    if (!res.ok)
      throw new Error(`שגיאה בטעינת תגובות (${res.status})`);

    const json = await res.json();

    const comments = (json.data || []).map((item) => {
      // ⚙️ אם אין attributes – נשתמש ישירות בפרופס הראשיים
      const a = item.attributes || item;

      return {
        id: item.id,
        author: a.author || a.user?.username || "אנונימי",
        text: a.text || a.content || "— אין תוכן —",
...
export async function addCommentByThreadSlug({
  ...
  try {
    ...
    try {
      // משיכת כל התגובות הקשורות לדיון
      ...
      // ✅ עדכון השדה commentsCount ב-Strapi גם אם הוא null
