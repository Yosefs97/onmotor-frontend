...
/* 🟨 קבלת שרשור בודד לפי slug (תומך בתוספות כמו -2, -3 וכו') */
export async function fetchThreadBySlug(threadSlug) {
  try {
    ...
    // ✅ במידה ויש תוספת כמו -2, נוריד אותה
    const normalizedSlug = decodedSlug.replace(/-\d+$/, "");
    console.log("🔍 slug לאחר ניקוי:", normalizedSlug);
    ...
    const url = `${API_URL}/api/forum-threads?${query.toString()}`;
    console.log("🌐 קריאה ל:", url);

    const res = await fetch(url, { cache: "no-store" });

    if (!res.ok) {
      console.error("❌ שגיאה בטעינת דיון לפי slug:", res.status, res.statusText);
      throw new Error("שגיאה בטעינת דיון לפי slug");
    }

    const json = await res.json();
    console.log("📦 תגובת Strapi:", json);

    const item = json.data?.[0];
    if (!item) {
      console.warn("⚠️ לא נמצא דיון עם slug:", normalizedSlug);
    ...
    console.error("❌ fetchThreadBySlug error:", err);
    throw err;
  }
}
