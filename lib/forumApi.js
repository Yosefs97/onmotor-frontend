  ...
  return (
    json.data?.map((item) => {
      ...
      return {
        ...
        commentsCount: attrs.comments?.length || attrs.comments?.data?.length || 0,
        category:
...
export async function fetchThreadsByCategorySlug(slug) {
  try {
    const res = await fetch(
      `${API_URL}/api/forum-threads?filters[category][slug][$eq]=${slug}&populate=comments`,
      { cache: 'no-store' }
    );
    ...
    return (
      json.data?.map((item) => {
        ...
        return {
          ...
          title: attrs.title || item.title,
          slug: attrs.slug || item.slug,
          author: attrs.author || item.author || 'אנונימי',
          content:
            Array.isArray(attrs.content) && attrs.content.length > 0
              ? attrs.content
                  .map((block) =>
                    block.children?.map((c) => c.text || '').join(' ')
                  )
          ...
          commentsCount:
            ...
            attrs.comments?.length ||
            0, // ✅ מספר תגובות חי מה-API
          date: attrs.date || attrs.createdAt || null,
          lastActivity: attrs.lastActivity || attrs.updatedAt || null,
        };
      }) || []
    );
  } catch (err) {
    console.error('❌ fetchThreadsByCategorySlug error:', err);
    return [];
  }
}
