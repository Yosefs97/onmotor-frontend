...
export async function fetchCommentsByThreadSlug(threadSlug) {
  try {
    ...
    const query = new URLSearchParams({
      ...
      "populate[comments][populate][reply_to][fields][0]": "id",
      "populate[comments][fields][0]": "author",
      "populate[comments][fields][1]": "text",
      "populate[comments][fields][2]": "date",
      "populate[comments][fields][3]": "createdAt",
      "sort[comments][0]": "createdAt:asc",
    });

    const url = `${process.env.NEXT_PUBLIC_STRAPI_API_URL}/api/forum-threads?${query.toString()}`;
    console.log("🌐 בקשה ל-Strapi:", url);

    const res = await fetch(url, { cache: "no-store" });
    if (!res.ok) throw new Error(`שגיאה בטעינת תגובות (${res.status})`);

    const json = await res.json();
    const thread = json.data?.[0];
    const commentsData = thread?.attributes?.comments?.data || [];

    // 🧩 בנייה למבנה הנכון שה־UI שלך מחפש
    const comments = commentsData.map((item) => ({
      id: item.id,
      author: item.author || item.attributes?.author || "אנונימי",
      text: item.text || item.attributes?.text || "— אין תוכן —",
      date: item.date || item.attributes?.date || item.attributes?.createdAt,
      createdAt: item.attributes?.createdAt,
      reply_to: item.reply_to?.id || item.attributes?.reply_to?.data?.id || null,
    }));

    console.log("💬 תגובות שהתקבלו מ-Strapi:", comments);
    return comments;
  } catch (err) {
