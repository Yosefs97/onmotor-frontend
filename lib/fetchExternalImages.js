// lib/fetchExternalImages.js
import * as cheerio from 'cheerio';

/**
 * גרסה אוניברסלית עם תמיכה ב-KTM, Yamaha, ודפים דינמיים
 * כוללת את כל הסינונים שלך מהקובץ הישן + מנגנון חכם חדש
 */
export async function fetchExternalImages(pageUrl) {
  try {
    console.log(`🔍 Fetching images from ${pageUrl}`);

    const response = await fetch(pageUrl, {
      headers: {
        'User-Agent':
          'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127 Safari/537.36',
      },
    });

    if (!response.ok)
      throw new Error(`HTTP ${response.status} when fetching ${pageUrl}`);

    const html = await response.text();
    const $ = cheerio.load(html);
    const baseUrl = new URL(pageUrl).origin;
    const found = new Set();

    // 🧩 פונקציה לאיחוד נתיבים ל-URL מלא
    const normalizeUrl = (src) => {
      if (!src) return null;
      if (src.startsWith('http')) return src;
      if (src.startsWith('//')) return `https:${src}`;
      if (src.startsWith('data:')) return null;
      return `${baseUrl}/${src.replace(/^\/+/, '')}`;
    };

    // 1️⃣ תמונות רגילות ו-lazy
    $('img').each((_, el) => {
      const src =
        $(el).attr('src') ||
        $(el).attr('data-src') ||
        $(el).attr('data-lazy') ||
        $(el).attr('data-original');
      const full = normalizeUrl(src);
      if (full) found.add(full);
    });

    // 2️⃣ רקעים מתוך style="background-image"
    $('[style]').each((_, el) => {
      const style = $(el).attr('style');
      const match = style?.match(/url\(['"]?(.*?)['"]?\)/i);
      if (match && match[1]) {
        const full = normalizeUrl(match[1]);
        if (full) found.add(full);
      }
    });

    // 3️⃣ קישורים ישירים לתמונות
    $('a[href]').each((_, el) => {
      const href = $(el).attr('href');
      if (href && /\.(jpg|jpeg|png|gif|webp|avif)$/i.test(href)) {
        const full = normalizeUrl(href);
        if (full) found.add(full);
      }
    });

    // 4️⃣ meta tags (תמונות ראשיות)
    $('meta[property="og:image"], meta[name="twitter:image"]').each((_, el) => {
      const content = $(el).attr('content');
      const full = normalizeUrl(content);
      if (full) found.add(full);
    });

    // 5️⃣ data attributes למקרה של lazy-load
    $('[data-bg],[data-srcset],[data-image],[data-lazy-src]').each((_, el) => {
      const attr =
        $(el).attr('data-bg') ||
        $(el).attr('data-srcset') ||
        $(el).attr('data-image') ||
        $(el).attr('data-lazy-src');
      const full = normalizeUrl(attr);
      if (full) found.add(full);
    });

    // 🟣 Yamaha Mode – חיפוש דינמי לפי year + model
    if (pageUrl.includes('yamaha-motor.eu')) {
      console.log('⚙️ Detected Yamaha site – applying CDN pattern');
      const yamahaMatch = pageUrl.match(/pdp\/([^/]+)-(\d{4})/i);

      if (yamahaMatch) {
        const model = yamahaMatch[1].toUpperCase().replace(/-/g, '');
        const year = yamahaMatch[2];
        const cdnPrefix = `https://cdn2.yamaha-motor.eu/prod/product-assets/${year}/${model}`;
        console.log(`📦 Yamaha model detected: ${model}, year: ${year}`);

        const cdnMatches = html.match(
          new RegExp(
            `${cdnPrefix.replace(/\//g, '\\/')}[^"']+\\.(jpg|jpeg|png|webp|avif)`,
            'gi'
          )
        );

        if (cdnMatches?.length) {
          cdnMatches.forEach((src) => found.add(src));
        } else {
          console.log('⚠️ No direct CDN matches found – scanning all Yamaha cdn2 links');
          const backupMatches = html.match(
            /(https:\/\/cdn2\.yamaha-motor\.eu\/prod\/product-assets\/[^\s"']+\.(jpg|jpeg|png|webp|avif))/gi
          );
          if (backupMatches?.length) {
            backupMatches.forEach((src) => found.add(src));
          }
        }
      }
    }

    // 🟠 KTM fallback – אם אין תוצאות רגילות
    if (found.size === 0 && pageUrl.includes('press.ktm.com')) {
      console.log('⚙️ Using KTM pattern fallback');
      const matches = html.match(
        /Content\/\d+\/[a-z0-9-]+\/1200\/2400\/\.jpg/gi
      );
      if (matches?.length) {
        matches.forEach((p) => {
          const fixed = `${baseUrl}/${p.replace(/^Content\//, '')}`;
          found.add(fixed);
        });
      }
    }

    // ✅ סינון סופי — כולל כל הפילטרים שלך
    const images = Array.from(found)
      .filter((src) =>
        /\.(jpg|jpeg|png|gif|webp|avif)$/i.test(src.toLowerCase())
      )
      .filter((src) => !src.toLowerCase().includes('loading.gif'))
      .filter((src) => !src.toLowerCase().includes('/assets/img/0.gif'))
      .filter((src) => !src.toLowerCase().includes('/assets/img/logo.png'))
      .filter((src) => !src.toLowerCase().includes('/assets/img/icon_pdf.png'))
      .filter((src) => !src.toLowerCase().includes('placeholder'))
      .map((src) => ({ src, alt: '', title: '' }));

    console.log(`✅ Found ${images.length} images from ${pageUrl}`);
    return images;
  } catch (err) {
    console.error('❌ fetchExternalImages error:', err);
    return [];
  }
}
