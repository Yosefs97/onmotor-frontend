 ...
 * - KTM press
 * - Honda News (כולל עטיפת proxy אוטומטית)
...
export async function fetchExternalImages(pageUrl) {
  try {
    ...
    // 🟣 טיפול בקישורים של Yamaha Media Library (inriver_preview)
    if (pageUrl.includes('medialibrary.yamaha-motor.eu/plugins/inriver_preview/asset/')) {
      console.log('⚙️ Yamaha Media Library asset detected');

      // חילוץ מזהה הנכס
      const match = pageUrl.match(/asset\/([^/]+)\//);
      if (match) {
        const assetId = match[1];
        const cdnUrl = `https://cdn2.yamaha-motor.eu/prod/product-assets/${assetId}/original.jpg`;
        console.log(`✅ Reconstructed Yamaha CDN URL: ${cdnUrl}`);
        return [{ src: cdnUrl, alt: '', title: '' }];
      }

      // fallback אם לא הצליח
    ...
    if (pageUrl.includes('hondanews.eu')) {
      ...
      // אם מדובר בעמוד מדיה (לא קובץ תמונה ישיר)
      if (pageUrl.includes('/media/photos/')) {
        console.log('🧭 Honda media photo page detected, parsing HTML to extract image...');
        try {
          const response = await fetch(pageUrl, {
            headers: {
              'User-Agent':
          ...
          if (response.ok) {
            ...
            if (match) {
              ...
              console.log(`✅ Found direct Honda image: ${imageUrl}`);
              const proxyUrl = `${proxyBase}/api/proxy-honda?url=${encodeURIComponent(imageUrl)}`;
              return [{ src: proxyUrl, alt: '', title: '' }];
            } else {
              console.warn('⚠️ No image URL found inside Honda media page');
            }
          } else {
            console.warn(`⚠️ Failed to fetch Honda media page: ${response.status}`);
          }
        } catch (err) {
          console.error('❌ Error parsing Honda media page:', err);
        }
      }

      // אחרת – עטיפה אוטומטית
      const proxyUrl = `${proxyBase}/api/proxy-honda?url=${encodeURIComponent(pageUrl)}`;
      console.log(`🖼️ Direct Honda image URL, wrapped with proxy: ${proxyUrl}`);
      return [{ src: proxyUrl, alt: '', title: '' }];
    }
