...
export async function fetchExternalImages(pageUrl) {
  try {
    ...
    if (
      ...
      // נחליף אוטומטית מ-low ל-high אם יש קובץ איכותי יותר
      const highRes = pageUrl.replace('/low/', '/high/');
      const originalRes = pageUrl.replace('/low/', '/original/');
      return [
        { src: originalRes, alt: '', title: '' },
        { src: highRes, alt: '', title: '' },
        { src: pageUrl, alt: '', title: '' },
      ];
    }
