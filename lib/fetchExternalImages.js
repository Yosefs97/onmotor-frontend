...
/**
 * ✅ תומך כעת ב:
 * - Yamaha Media (release-overview, REST API אמיתי)
...
export async function fetchExternalImages(pageUrl) {
  try {
    ...
    if (pageUrl.includes('media.yamaha-motor.eu')) {
      console.log('⚙️ Yamaha Media site detected — using REST API');
      ...
      if (slug) {
        const apiUrl = `https://media.yamaha-motor.eu/api/media-items?filters[release][slug][$eq]=${slug}&populate=*`;
