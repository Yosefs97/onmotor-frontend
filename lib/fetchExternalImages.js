...
export async function fetchExternalImages(pageUrl) {
  try {
    ...
    const found = new Set();
      ...
      // 🖼️ טיפול בקישורי תמונה ישירים (כולל Honda News)
    if (
      pageUrl.match(/\.(jpg|jpeg|png|webp|gif|avif)(\?|$)/i) ||
      pageUrl.includes('/image/motorcycles/')
    ) {
      console.log('🖼️ Direct image URL detected (e.g. Honda News)');
      try {
        ...
        // 🎯 נוודא שהשרת לא החזיר HTML (שזו בעצם תמונת הלוגו או fallback)
        const contentType = res.headers.get('content-type') || '';
        if (!contentType.startsWith('image/')) {
          console.warn('⚠️ Honda returned non-image content, skipping.');
          return [];
        }
