...
// 🌟 הוספנו את productTags לפרופס 🌟
export default function RelatedProducts({ partVendor, productType, compatibleModels, excludeHandle, productTags = [] }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      // 🌟 מוסיפים את התגיות לפרמטרים של החיפוש 🌟
      if (productTags && productTags.length > 0) {
        // מסננים תגיות טכניות (כמו fit: או אלו שמתחילות באותיות שמורות) אם צריך
        // במקרה הזה נשלח את כולן כרשימה מופרדת בפסיקים
  ...
  return (
    <div dir="rtl" className="mt-8 border-t border-gray-100 pt-6">
      ...
      <div className="flex flex-col xl:flex-row xl:justify-between xl:items-end gap-3 mb-4">
        {/* שינינו את הכותרת ל"מוצרים דומים" */}
        <h3 className="font-bold text-xl text-gray-900 pr-1">מוצרים דומים</h3>
      ...
      <div className="relative group">
        {items.length > 2 && (
          <>
            <button onClick={() => scroll('prev')} className={`${arrowClasses} right-0 translate-x-1/3 hidden md:block`} aria-label="הקודם">
            ...
            <button onClick={() => scroll('next')} className={`${arrowClasses} left-0 -translate-x-1/3 hidden md:block`} aria-label="הבא">
        ...
        <div ref={scrollRef} className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-4 hide-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {items.map((p) => {
            const img = p.images?.edges?.[0]?.node?.url;
            const price = p.variants?.edges?.[0]?.node?.price;
            return (
              <Link key={p.id} href={`/shop/${p.handle}`} prefetch={false} className="border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-shadow flex-shrink-0 snap-start bg-white w-[160px] md:w-[180px]">
                {img ? (
