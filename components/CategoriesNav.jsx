        ...
        <div className="flex items-center px-4 h-full">
          {categories.map((cat, index) => (
            <div key={cat.handle || cat.title} className="flex items-center shrink-0 h-full">
              ...
              {/* קו מפריד */}
              {index > 0 && <div className="w-px h-3 bg-[#e60000] mx-3" />}

              <Link
                href={cat.href || `/collections/${cat.handle}`}
                className="text-gray-800 hover:!text-[#e60000] text-sm font-bold whitespace-nowrap transition-colors flex items-center"
              >
                {cat.title}
              </Link>
            </div>
      ...
      <div className="hidden md:flex items-center px-2">
         {categories.map((cat, index) => (
          <div key={cat.handle || cat.title} className="flex items-center">
            {index > 0 && <div className="w-px h-4 bg-[#e60000] mx-3" />}
            <Link
              href={cat.href || `/collections/${cat.handle}`}
              className="text-base font-bold text-gray-700 hover:!text-[#e60000] transition-colors whitespace-nowrap"
            >
              {cat.title}
            </Link>
          </div>
