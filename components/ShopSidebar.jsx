...
export default function ShopSidebar({ 
  ...
  const [facets, setFacets] = useState({ vendors: [], models: {}, yearsByModel: {}, categoriesByModel: {} });
  const [filters, setFilters] = useState({ q: '', sku: '', vendor: '', model: '', yearFrom: '', yearTo: '', category: '' });
  ...
  useEffect(() => {
    ...
    if (list.length) {
      ...
      setYearRange([min, max]); setCursor([min, max]);
      setFilters(f => ({ ...f, yearFrom: String(min), yearTo: String(max) }));
      setAutoOpenYear(true); setTimeout(() => scrollToElement("filter-year"), 150);
    }
  ...
  const handleVendorChange = (val) => {
    setFilters((f) => ({ ...f, vendor: val, model: '', yearFrom: '', yearTo: '', category: '', q: '', sku: '' }));
    setAutoOpenModel(true); setTimeout(() => scrollToElement("filter-model"), 150);
  };
    ...
    >
      ...
      <div className="space-y-1 relative" ref={searchWrapperRef}>
        ...
        <div className="relative">
            <input type="text" placeholder="לדוגמה: פילטר שמן או מק'ט" value={filters.q}
              onChange={(e) => setFilters((f) => ({ ...f, q: e.target.value, vendor: '', model: '', yearFrom: '', yearTo: '', category: '' }))}
            ...
            <div className="absolute top-full right-0 left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-xl z-50 max-h-60 overflow-y-auto">
                <ul className="divide-y divide-gray-100">
                    {suggestions.map((item) => (
                        <li key={item.id}>
                            <Link href={`/shop/${item.handle}`} className="flex items-center gap-3 p-2 hover:bg-gray-50 transition duration-150" onClick={() => setShowDropdown(false)}>
                                <div className="w-10 h-10 flex-shrink-0 bg-gray-100 rounded overflow-hidden border border-gray-200">
                                    {item.image ? (<img src={item.image} alt={item.title} className="w-full h-full object-cover" />) : (<div className="w-full h-full flex items-center justify-center text-xs text-gray-400">אין</div>)}
                                </div>
                                <div className="flex-1 min-w-0 text-right">
                                    <p className="text-sm font-bold text-gray-900 truncate">{item.title}</p>
                                    <p className="text-xs text-gray-500 truncate">{item.type}</p>
                                </div>
                                <div className="text-sm font-bold text-red-600 whitespace-nowrap pl-1">₪{parseInt(item.price)}</div>
                            </Link>
                        </li>
                    ))}
                </ul>
      ...
      {filters.vendor && ( <DropdownSimple id="filter-model" label="דגם" value={filters.model} options={facets.models[filters.vendor] || []} onChange={(val) => { setFilters((f) => ({ ...f, model: val, category: '' })); setAutoOpenYear(true); setTimeout(() => scrollToElement("filter-year"), 150); }} forceOpen={autoOpenModel} /> )}
      {filters.model && yearRange[0] > 0 && (
        <div id="filter-year" className="space-y-2">
          <label className="text-lg font-bold">טווח שנים</label>
          <div className="flex gap-2"><input type="range" min={yearRange[0]} max={yearRange[1]} value={cursor[0]} onChange={(e) => setCursor(([_, r]) => [Math.min(parseInt(e.target.value), r), r])} className="w-full" /><span>{cursor[0]}</span></div>
          <div className="flex gap-2"><input type="range" min={yearRange[0]} max={yearRange[1]} value={cursor[1]} onChange={(e) => setCursor(([l, _]) => [l, Math.max(parseInt(e.target.value), l)])} className="w-full" /><span>{cursor[1]}</span></div>
          <div className="text-xs text-gray-600">נבחר: {cursor[0]} - {cursor[1]}</div>
        </div>
      )}
      {filters.model && ( <DropdownSimple id="filter-category" label="קטגוריה" value={filters.category} options={facets.categoriesByModel[filters.model] || []} onChange={(val) => { setFilters((f) => ({ ...f, category: val })); setAutoOpenCategory(true); setTimeout(() => scrollToElement("filter-submit-btn"), 150); }} forceOpen={autoOpenCategory} /> )}
